...
class SmartViewController: UIViewController {
  ...
  let warpGeometryFilter: WarpGeometryFilter = .init(device: MTLCreateSystemDefaultDevice()!)
...
extension SmartViewController: ARSessionDelegate {
  func session(_ session: ARSession, didUpdate frame: ARFrame) {
    ...
    if let face = detector.faces.first, let perspectivePoints = face.perspectivePoints, toggle.isOn {
      ...
      let cgImage = context.createCGImage(compositedOutput, from: compositedOutput.extent)!
